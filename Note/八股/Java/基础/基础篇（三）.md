...
- **`Error`**：`Error` 属于**程序无法处理的错误** ，~~我们没办法通过 `catch` 来进行捕获~~不建议通过`catch`捕获 。例如 Java 虚拟机运行错误（`Virtual MachineError`）、虚拟机内存不够错误(`OutOfMemoryError`)、类定义错误（`NoClassDefFoundError`）等 。这些异常发生时，Java 虚拟机（JVM）一般会选择线程终止。
...
**`try-with-resources`** 的写法就是在 `try` 关键字**后面的括号中声明和初始化要使用的资源**，并且这些资源必须实现 **`AutoCloseable`** 或 **`Closeable`** 接口（这两个接口都包含 `close()` 方法，用于自动关闭资源）。

《Effective Java》中明确指出：

> 面对**必须要关闭的资源**，我们总是应该优先使用 `try-with-resources` 而不是`try-finally`。随之产生的代码更简短，更清晰，产生的异常对我们也更有用。`try-with-resources`语句让我们更容易编写必须要关闭的资源的代码，若采用`try-finally`则几乎做不到这点。
...
关于反射的详细解读，请看这篇文章 [Java 反射机制详解](https://javaguide.cn/java/basis/reflection.html) 。也可以参考[[JavaSE 笔记（六）多线程与反射#反射]]。
...
`Annotation` （注解） 是 Java5 开始引入的新特性，可以看作是一种特殊的注释，主要用于修饰类、方法或者变量，提供某些信息供程序在编译或者运行时使用。可以参考[[JavaSE 笔记（六）多线程与反射#注解]]。
...
关于 I/O 的详细解读，请看下面这几篇文章，里面涉及到的知识点和面试题更全面。也可以参考[[JavaSE 笔记（五）集合类与IO#Java I/O]]。
...
IO 即 `Input/Output`，输入和输出。数据输入到计算机内存的过程即输入，反之输出到外部存储（比如数据库，文件，远程主机）的过程即输出。数据传输过程类似于水流，因此称为 IO 流。IO 流在 Java 中分为输入流和输出流，而根据数据的处理方式又分为**字节流和字符流**。
